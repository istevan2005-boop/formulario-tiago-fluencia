...
export default function Home() {
  ...
  // Se a pessoa apertar "voltar" no navegador em qualquer etapa do formulário,
  // ela é levada direto pra tela do grupo (a LP) em vez de sair do formulário.
  // Se ela apertar "voltar" de novo já estando na tela do grupo, aí sim
  // segue pro link externo do WhatsApp.
  useEffect(() => {
    ...
    const handlePopState = () => {
      setStatus((current) => {
        if (current === "done") {
          window.location.href = groupDestination;
          return current;
        }
        window.history.pushState(null, "", window.location.href);
        return "done";
  ...
  // Enquanto a pessoa ainda está respondendo o formulário, mostra o aviso
  // nativo do navegador se ela tentar fechar a aba ou sair do site.
  // (Não é possível redirecionar nesse caso, só avisar — os navegadores
  // bloqueiam redirect forçado ao fechar aba por segurança.)
  useEffect(() => {
